...
DIMENSIONS = (3, 3, 3)
RANK = 23
CERTIFICATE_SHA256 = "f63c6177f31df3f8867196f40439afd17f1c0463ac950bc6891ee40ed38596b6"
...
def left_transform_kernel(
    ...
    tl.store(
        output + 2 * plane_elements + element_offsets, signal_5, mask=element_mask
    )
    signal_4 = tl.load(
    ...
    tl.store(
        output + 5 * plane_elements + element_offsets, signal_4, mask=element_mask
    )
    signal_8 = tl.load(
    ...
    tl.store(
        output + 6 * plane_elements + element_offsets, signal_8, mask=element_mask
    )
    tl.store(
        output + 8 * plane_elements + element_offsets, signal_4, mask=element_mask
    )
    tl.store(
        output + 11 * plane_elements + element_offsets, signal_8, mask=element_mask
    )
    signal_2 = tl.load(
    ...
    tl.store(
        output + 13 * plane_elements + element_offsets, signal_2, mask=element_mask
    )
    signal_1 = tl.load(
    ...
    tl.store(
        output + 17 * plane_elements + element_offsets, signal_0, mask=element_mask
    )
    tl.store(
        output + 18 * plane_elements + element_offsets, signal_5, mask=element_mask
    )
    signal_3 = tl.load(
    ...
    tl.store(
        output + 19 * plane_elements + element_offsets, signal_3, mask=element_mask
    )
    signal_7 = tl.load(
    ...
    tl.store(
        output + 21 * plane_elements + element_offsets, signal_9, mask=element_mask
    )
    signal_10 = signal_4 - signal_9
    tl.store(
        output + 7 * plane_elements + element_offsets, -signal_10, mask=element_mask
    )
    signal_11 = signal_1 - signal_10
    tl.store(
        output + 9 * plane_elements + element_offsets, signal_11, mask=element_mask
    )
    signal_12 = signal_11 - signal_6
    tl.store(
        output + 10 * plane_elements + element_offsets, -signal_12, mask=element_mask
    )
    signal_13 = signal_12 - signal_8
    tl.store(
        output + 4 * plane_elements + element_offsets, signal_13, mask=element_mask
    )
    signal_14 = signal_1 - signal_13
    tl.store(
        output + 0 * plane_elements + element_offsets, signal_14, mask=element_mask
    )
    signal_15 = signal_0 - signal_11
    tl.store(
        output + 12 * plane_elements + element_offsets, signal_15, mask=element_mask
    )
    signal_16 = signal_13 + signal_2
    tl.store(
        output + 3 * plane_elements + element_offsets, signal_16, mask=element_mask
    )
    signal_17 = signal_15 - signal_3
    tl.store(
        output + 16 * plane_elements + element_offsets, signal_17, mask=element_mask
    )
    signal_18 = signal_17 + signal_5
    tl.store(
        output + 22 * plane_elements + element_offsets, signal_18, mask=element_mask
    )
    signal_19 = signal_18 - signal_2
    tl.store(
        output + 20 * plane_elements + element_offsets, -signal_19, mask=element_mask
    )
    signal_20 = signal_0 - signal_1
    signal_21 = signal_20 - signal_18
    tl.store(
        output + 1 * plane_elements + element_offsets, signal_21, mask=element_mask
    )
...
def right_transform_kernel(
    ...
    tl.store(
        output + 3 * plane_elements + element_offsets, signal_8, mask=element_mask
    )
    signal_5 = tl.load(
        source + (1 * block_height + local_rows) * source_columns + 2 * block_width + local_columns, mask=element_mask & (1 * block_height + local_rows < source_row_count) & (2 * block_width + local_columns < source_columns), other=0.0
    ).to(tl.float32)
    ...
    tl.store(
        output + 6 * plane_elements + element_offsets, signal_7, mask=element_mask
    )
    signal_3 = tl.load(
        source + (1 * block_height + local_rows) * source_columns + 0 * block_width + local_columns, mask=element_mask & (1 * block_height + local_rows < source_row_count) & (0 * block_width + local_columns < source_columns), other=0.0
    ).to(tl.float32)
    ...
    tl.store(
        output + 13 * plane_elements + element_offsets, signal_6, mask=element_mask
    )
    tl.store(
    ...
    tl.store(
        output + 18 * plane_elements + element_offsets, signal_6, mask=element_mask
    )
    tl.store(
        output + 19 * plane_elements + element_offsets, signal_0, mask=element_mask
    )
    tl.store(
        output + 20 * plane_elements + element_offsets, signal_7, mask=element_mask
    )
    signal_4 = tl.load(
        source + (1 * block_height + local_rows) * source_columns + 1 * block_width + local_columns, mask=element_mask & (1 * block_height + local_rows < source_row_count) & (1 * block_width + local_columns < source_columns), other=0.0
    ).to(tl.float32)
    ...
    signal_9 = signal_2 + signal_5
    signal_10 = signal_3 - signal_5
    signal_1 = tl.load(
        source + (0 * block_height + local_rows) * source_columns + 1 * block_width + local_columns, mask=element_mask & (0 * block_height + local_rows < source_row_count) & (1 * block_width + local_columns < source_columns), other=0.0
    ).to(tl.float32)
    signal_11 = signal_1 + signal_4
    tl.store(
        output + 15 * plane_elements + element_offsets, signal_11, mask=element_mask
    )
    signal_12 = signal_8 - signal_9
    tl.store(
        output + 10 * plane_elements + element_offsets, -signal_12, mask=element_mask
    )
    signal_13 = signal_0 - signal_11
    tl.store(
        output + 9 * plane_elements + element_offsets, signal_13, mask=element_mask
    )
    signal_14 = signal_2 - signal_13
    tl.store(
        output + 7 * plane_elements + element_offsets, signal_14, mask=element_mask
    )
    signal_15 = signal_4 - signal_14
    tl.store(
        output + 1 * plane_elements + element_offsets, -signal_15, mask=element_mask
    )
    signal_16 = signal_7 - signal_15
    tl.store(
        output + 22 * plane_elements + element_offsets, -signal_16, mask=element_mask
    )
    signal_17 = signal_2 - signal_16
    tl.store(
        output + 16 * plane_elements + element_offsets, signal_17, mask=element_mask
    )
    signal_18 = signal_8 + signal_16
    tl.store(
        output + 2 * plane_elements + element_offsets, signal_18, mask=element_mask
    )
    signal_19 = signal_10 - signal_14
    tl.store(
        output + 0 * plane_elements + element_offsets, -signal_19, mask=element_mask
    )
    signal_20 = signal_8 + signal_19
    tl.store(
        output + 4 * plane_elements + element_offsets, -signal_20, mask=element_mask
    )
    signal_21 = signal_6 - signal_20
    tl.store(
        output + 11 * plane_elements + element_offsets, signal_21, mask=element_mask
    )
...
def output_transform_kernel(
    ...
    tl.store(
        output + (0 * block_height + local_rows) * output_columns + 0 * block_width + local_columns, signal_13 + signal_14 + signal_17, mask=element_mask & (0 * block_height + local_rows < output_row_count) & (0 * block_width + local_columns < output_columns)
    )
    signal_8 = tl.load(
    ...
    tl.store(
        output + (1 * block_height + local_rows) * output_columns + 0 * block_width + local_columns, signal_8 + signal_18 + signal_19, mask=element_mask & (1 * block_height + local_rows < output_row_count) & (0 * block_width + local_columns < output_columns)
    )
    signal_6 = tl.load(
    ...
    tl.store(
        output + (2 * block_height + local_rows) * output_columns + 1 * block_width + local_columns, signal_6 + signal_15 - signal_21, mask=element_mask & (2 * block_height + local_rows < output_row_count) & (1 * block_width + local_columns < output_columns)
    )
    signal_4 = tl.load(
    ...
    signal_24 = signal_12 + signal_22
    signal_25 = signal_14 + signal_23
    signal_0 = tl.load(
    ...
    signal_26 = signal_0 + signal_7
    signal_3 = tl.load(
        source + 3 * plane_elements + element_offsets, mask=element_mask, other=0.0
    ).to(tl.float32)
    tl.store(
        output + (0 * block_height + local_rows) * output_columns + 2 * block_width + local_columns, signal_3 + signal_12 + signal_25 + signal_26, mask=element_mask & (0 * block_height + local_rows < output_row_count) & (2 * block_width + local_columns < output_columns)
    )
    signal_1 = tl.load(
        source + 1 * plane_elements + element_offsets, mask=element_mask, other=0.0
    ).to(tl.float32)
    signal_27 = signal_1 - signal_7
    signal_20 = tl.load(
        source + 20 * plane_elements + element_offsets, mask=element_mask, other=0.0
    ).to(tl.float32)
    tl.store(
        output + (0 * block_height + local_rows) * output_columns + 1 * block_width + local_columns, -signal_9 + signal_17 + signal_20 - signal_24 + signal_27, mask=element_mask & (0 * block_height + local_rows < output_row_count) & (1 * block_width + local_columns < output_columns)
    )
    signal_10 = tl.load(
        source + 10 * plane_elements + element_offsets, mask=element_mask, other=0.0
    ).to(tl.float32)
    signal_28 = signal_10 + signal_25
    signal_11 = tl.load(
        source + 11 * plane_elements + element_offsets, mask=element_mask, other=0.0
    ).to(tl.float32)
    tl.store(
        output + (2 * block_height + local_rows) * output_columns + 0 * block_width + local_columns, -signal_8 + signal_11 + signal_15 + signal_28, mask=element_mask & (2 * block_height + local_rows < output_row_count) & (0 * block_width + local_columns < output_columns)
    )
    signal_5 = tl.load(
        source + 5 * plane_elements + element_offsets, mask=element_mask, other=0.0
    ).to(tl.float32)
    tl.store(
        output + (2 * block_height + local_rows) * output_columns + 2 * block_width + local_columns, -signal_5 + signal_26 + signal_28, mask=element_mask & (2 * block_height + local_rows < output_row_count) & (2 * block_width + local_columns < output_columns)
    )
    signal_16 = tl.load(
        source + 16 * plane_elements + element_offsets, mask=element_mask, other=0.0
    ).to(tl.float32)
    signal_29 = signal_16 - signal_24
    tl.store(
        output + (1 * block_height + local_rows) * output_columns + 1 * block_width + local_columns, signal_19 + signal_21 + signal_27 + signal_29, mask=element_mask & (1 * block_height + local_rows < output_row_count) & (1 * block_width + local_columns < output_columns)
    )
    signal_2 = tl.load(
        source + 2 * plane_elements + element_offsets, mask=element_mask, other=0.0
    ).to(tl.float32)
    tl.store(
        output + (1 * block_height + local_rows) * output_columns + 2 * block_width + local_columns, signal_2 + signal_5 - signal_29, mask=element_mask & (1 * block_height + local_rows < output_row_count) & (2 * block_width + local_columns < output_columns)
    )
...
def output_transform_bias_kernel(
    ...
    tl.store(
        output + (0 * block_height + local_rows) * output_columns + 0 * block_width + local_columns, signal_13 + signal_14 + signal_17 + bias_0, mask=element_mask & (0 * block_height + local_rows < output_row_count) & (0 * block_width + local_columns < output_columns)
    )
    signal_8 = tl.load(
        source + 8 * plane_elements + element_offsets, mask=element_mask, other=0.0
    ).to(tl.float32)
    ...
    tl.store(
        output + (1 * block_height + local_rows) * output_columns + 0 * block_width + local_columns, signal_8 + signal_18 + signal_19 + bias_1, mask=element_mask & (1 * block_height + local_rows < output_row_count) & (0 * block_width + local_columns < output_columns)
    )
    signal_6 = tl.load(
        source + 6 * plane_elements + element_offsets, mask=element_mask, other=0.0
    ).to(tl.float32)
    ...
    tl.store(
        output + (2 * block_height + local_rows) * output_columns + 1 * block_width + local_columns, signal_6 + signal_15 - signal_21 + bias_5, mask=element_mask & (2 * block_height + local_rows < output_row_count) & (1 * block_width + local_columns < output_columns)
    )
    signal_4 = tl.load(
        source + 4 * plane_elements + element_offsets, mask=element_mask, other=0.0
    ).to(tl.float32)
    ...
    tl.store(
        output + (0 * block_height + local_rows) * output_columns + 2 * block_width + local_columns, signal_3 + signal_12 + signal_25 + signal_26 + bias_6, mask=element_mask & (0 * block_height + local_rows < output_row_count) & (2 * block_width + local_columns < output_columns)
    )
    signal_1 = tl.load(
        source + 1 * plane_elements + element_offsets, mask=element_mask, other=0.0
    ).to(tl.float32)
    ...
    tl.store(
        output + (0 * block_height + local_rows) * output_columns + 1 * block_width + local_columns, -signal_9 + signal_17 + signal_20 - signal_24 + signal_27 + bias_3, mask=element_mask & (0 * block_height + local_rows < output_row_count) & (1 * block_width + local_columns < output_columns)
    )
    signal_10 = tl.load(
        source + 10 * plane_elements + element_offsets, mask=element_mask, other=0.0
    ).to(tl.float32)
    ...
    tl.store(
        output + (2 * block_height + local_rows) * output_columns + 0 * block_width + local_columns, -signal_8 + signal_11 + signal_15 + signal_28 + bias_2, mask=element_mask & (2 * block_height + local_rows < output_row_count) & (0 * block_width + local_columns < output_columns)
    )
    signal_5 = tl.load(
        source + 5 * plane_elements + element_offsets, mask=element_mask, other=0.0
    ).to(tl.float32)
    ...
    tl.store(
        output + (2 * block_height + local_rows) * output_columns + 2 * block_width + local_columns, -signal_5 + signal_26 + signal_28 + bias_8, mask=element_mask & (2 * block_height + local_rows < output_row_count) & (2 * block_width + local_columns < output_columns)
    )
    signal_16 = tl.load(
        source + 16 * plane_elements + element_offsets, mask=element_mask, other=0.0
    ).to(tl.float32)
    ...
    tl.store(
        output + (1 * block_height + local_rows) * output_columns + 1 * block_width + local_columns, signal_19 + signal_21 + signal_27 + signal_29 + bias_4, mask=element_mask & (1 * block_height + local_rows < output_row_count) & (1 * block_width + local_columns < output_columns)
    )
    signal_2 = tl.load(
        source + 2 * plane_elements + element_offsets, mask=element_mask, other=0.0
    ).to(tl.float32)
    ...
    tl.store(
        output + (1 * block_height + local_rows) * output_columns + 2 * block_width + local_columns, signal_2 + signal_5 - signal_29 + bias_7, mask=element_mask & (1 * block_height + local_rows < output_row_count) & (2 * block_width + local_columns < output_columns)
    )
